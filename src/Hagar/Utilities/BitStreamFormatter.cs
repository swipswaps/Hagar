...
namespace Hagar.Utilities
{
    public static class BitStreamFormatter
    {
        ...
        private static void AnalyzeField<TInput>(this ref Reader<TInput> reader, StringBuilder res)
        {
            ...
            AnalyzeField(ref reader, field, type, field.FieldIdDelta, res, indentation: 0);
        }

        public static void AnalyzeField<TInput>(this ref Reader<TInput> reader, Field field, string typeName, uint id, StringBuilder res, int indentation)
        {
            ...
            switch (field.WireType)
            {
                ...
                case WireType.TagDelimited:
                    ...
                    res.Append($"{{\n");
                    reader.AnalyzeTagDelimitedField(res, indentation + 1);
                    res.Append($"\n{indentString}}}");
                    break;
        ...
        private static void PrintByteArray(StringBuilder res, byte[] a)
        {
            var isAscii = true;
            foreach (var b in a)
            {
        ...
        private static void PrintField(StringBuilder res, SerializerSession session, Field field, uint id, string typeName)
        {
            _ = res
        ...
        /// </summary>
        private static void AnalyzeTagDelimitedField<TInput>(this ref Reader<TInput> reader, StringBuilder res, int indentation)
        {
            var id = 0U;
            var first = true;
            while (true)
            {
                ...
                reader.AnalyzeField(field, type, id, res, indentation);
            }
        }
    }
}
